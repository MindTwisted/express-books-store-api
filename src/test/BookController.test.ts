...
describe('booksController', () => {
    ...
    describe(`PUT ${BOOKS_URL}/:id`, () => {
        it('should response with 401 if auth token was not provided', async () => {
            const res = await agent.put(`${BOOKS_URL}/101`);
        ...
        it('should response with 403 if auth token was not owned by admin', async () => {
            ...
            const res = await agent.put(`${BOOKS_URL}/101`).set('Authorization', `Bearer ${token}`);

            res.should.have.status(403);
            res.body.should.have.property('status').eql('failed');
        });

        it('should response with 404 if book with such id does not exist', async () => {
            const token = (await agent.put(AUTH_URL).send({
                email: 'john@example.com',
                password: 'secret',
            })).body.body.data.token;
            const res = await agent.put(`${BOOKS_URL}/9999`).set('Authorization', `Bearer ${token}`);

            res.should.have.status(404);
            res.body.should.have.property('status').eql('failed');
        });

        it('should response with 422 if book title, description, price and discount were not provided', async () => {
            const token = (await agent.put(AUTH_URL).send({
                email: 'john@example.com',
                password: 'secret',
            })).body.body.data.token;
        ...
        it('should response with 422 if book title has already existed', async () => {
            const BOOK_TITLE = (await agent.get(`${BOOKS_URL}/100`)).body.body.data.book.title;
            const BOOK_DESCRIPTION = 'a'.repeat(20);
            const BOOK_PRICE = 10;
            const BOOK_DISCOUNT = 10;
        ...
        it('should response with 422 if book discount was less than zero', async () => {
            const BOOK_TITLE = 'a'.repeat(6);
            const BOOK_DESCRIPTION = 'a'.repeat(20);
            const BOOK_PRICE = 10;
            const BOOK_DISCOUNT = -1;
        ...
        it('should response with 200 and book with authors and genres if valid data were provided', async () => {
            ...
            const BOOK_PRICE = 10;
            const BOOK_DISCOUNT = 10;
            const BOOK_AUTHORS: any[] = [51, 52, 53, 'abc', 'cd'];
            const BOOK_GENRES: any[] = [54, 55, 'abc'];
